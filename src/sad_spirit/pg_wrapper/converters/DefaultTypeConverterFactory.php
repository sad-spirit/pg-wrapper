...
class DefaultTypeConverterFactory implements TypeConverterFactory, TypeOIDMapperAware
{
     ...
     *
     * This is for BC and only works if an instance of CachedTypeMapper is used for OID mapping
     *
     * @param bool $caching
     * @return $this
     * @deprecated Since 2.2.0, use {@see CachedTypeOIDMapper::setCompositeTypesCaching()} instead
     */
    final public function setCompositeTypesCaching(bool $caching): self
    {
        @trigger_error(sprintf(
            'The "%s()" method is deprecated since release 2.2.0, '
            . 'use "CachedTypeOIDMapper::setCompositeTypesCaching()" instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);

        if ($this->typeOIDMapper instanceof CachedTypeOIDMapper) {
            $this->typeOIDMapper->setCompositeTypesCaching($caching);
        }
     ...
     *
     * @return bool
     * @deprecated Since 2.2.0, use {@see CachedTypeOIDMapper::getCompositeTypesCaching()} instead
     */
    final public function getCompositeTypesCaching(): bool
    {
        @trigger_error(sprintf(
            'The "%s()" method is deprecated since release 2.2.0, '
            . 'use "CachedTypeOIDMapper::getCompositeTypesCaching()" instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);

        if ($this->typeOIDMapper instanceof CachedTypeOIDMapper) {
            return $this->typeOIDMapper->getCompositeTypesCaching();
        }
        return false;
    }
     ...
     * @return bool
     * @deprecated Since 2.2.0, use {@see TypeOIDMapper::isArrayTypeOID()} instead
     *
    ...
    final protected function isArrayTypeOID($oid, &$baseTypeOid = null): bool
    {
        @trigger_error(sprintf(
            'The "%s()" method is deprecated since release 2.2.0, '
            . 'use "TypeOIDMapper::isArrayTypeOID()" instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);

        return $this->getOIDMapper()->isArrayTypeOID($oid, $baseTypeOid);
    }
     ...
     * @param int|numeric-string|null $baseTypeOid
     * @return bool
     * @deprecated Since 2.2.0, use {@see TypeOIDMapper::isRangeTypeOID()} instead
     *
     * @psalm-assert-if-true int|numeric-string $baseTypeOid
     */
    final protected function isRangeTypeOID($oid, &$baseTypeOid = null): bool
    {
        @trigger_error(sprintf(
            'The "%s()" method is deprecated since release 2.2.0, '
            . 'use "TypeOIDMapper::isRangeTypeOID()" instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);

        return $this->getOIDMapper()->isRangeTypeOID($oid, $baseTypeOid);
    }
     ...
     * @param int|numeric-string|null $baseTypeOid
     * @return bool
     * @deprecated Since 2.2.0, use {@see TypeOIDMapper::isMultiRangeTypeOID()} instead
     *
     * @psalm-assert-if-true int|numeric-string $baseTypeOid
     */
    final protected function isMultiRangeTypeOID($oid, &$baseTypeOid = null): bool
    {
        @trigger_error(sprintf(
            'The "%s()" method is deprecated since release 2.2.0, '
            . 'use "TypeOIDMapper::isMultiRangeTypeOID()" instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);

        return $this->getOIDMapper()->isMultiRangeTypeOID($oid, $baseTypeOid);
    }
     ...
     * @param int|numeric-string|null $baseTypeOid
     * @return bool
     * @deprecated Since 2.2.0, use {@see TypeOIDMapper::isDomainTypeOID()} instead
     *
     * @psalm-assert-if-true int|numeric-string $baseTypeOid
     */
    final protected function isDomainTypeOID($oid, &$baseTypeOid = null): bool
    {
        @trigger_error(sprintf(
            'The "%s()" method is deprecated since release 2.2.0, '
            . 'use "TypeOIDMapper::isDomainTypeOID()" instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);

        return $this->getOIDMapper()->isDomainTypeOID($oid, $baseTypeOid);
    }
     ...
     * @param int|numeric-string $oid
     * @return bool
     * @deprecated Since 2.2.0, use {@see TypeOIDMapper::isCompositeTypeOID()} instead
     */
    final protected function isCompositeTypeOID($oid): bool
    {
        @trigger_error(sprintf(
            'The "%s()" method is deprecated since release 2.2.0, '
            . 'use "TypeOIDMapper::isCompositeTypeOID()" instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);

        return $this->getOIDMapper()->isCompositeTypeOID($oid);
    }
     ...
     * @param int|numeric-string $oid
     * @return bool
     * @deprecated Since 2.2.0, use {@see TypeOIDMapper::isBaseTypeOID()} instead
     */
    final protected function isBaseTypeOID($oid): bool
    {
        @trigger_error(sprintf(
            'The "%s()" method is deprecated since release 2.2.0, '
            . 'use "TypeOIDMapper::isBaseTypeOID()" instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);

        return $this->getOIDMapper()->isBaseTypeOID($oid);
    }
     ...
     * @throws InvalidArgumentException
     * @deprecated Since 2.2.0, use {@see TypeOIDMapper::findTypeNameForOID()} instead
     */
    final protected function findTypeNameForOID($oid, string $method): array
    {
        @trigger_error(sprintf(
            'The "%s()" method is deprecated since release 2.2.0, '
            . 'use "TypeOIDMapper::findTypeNameForOID()" instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);

        try {
            return $this->getOIDMapper()->findTypeNameForOID($oid);
        } catch (InvalidArgumentException $e) {
            throw new InvalidArgumentException($method . ': ' . $e->getMessage());
        }
    }
     ...
     * @throws InvalidArgumentException
     * @deprecated Since 2.2.0, use {@see TypeOIDMapper::findOIDForTypeName()} instead
     */
    final protected function findOIDForTypeName(string $typeName, ?string $schemaName, string $method)
    {
        @trigger_error(sprintf(
            'The "%s()" method is deprecated since release 2.2.0, '
            . 'use "TypeOIDMapper::findOIDForTypeName()" instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);

        try {
            return $this->getOIDMapper()->findOIDForTypeName($typeName, $schemaName);
        } catch (InvalidArgumentException $e) {
            throw new InvalidArgumentException($method . ': ' . $e->getMessage());
        }
    }
